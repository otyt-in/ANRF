# ANRF Website TODO

## Immediate

- [ ] Fix `/studio` and confirm embedded Sanity Studio loads on Vercel.
- [x] Add shared header and footer across public pages.
- [x] Add page-to-page CTAs so visitors are guided through the site.
- [ ] Create initial Sanity documents: Site Settings, Home Page and starter Stories.
- [ ] Fix Codex GitHub plugin access to private `otyt-in/ANRF` repository.
- [x] Add YouTube/Instagram link fields to Sanity site settings.
- [x] Add homepage social/media section.
- [x] Connect homepage Stories section to Sanity stories.
- [x] Connect homepage featured video URL to Sanity Home Page document.
- [x] Connect core homepage images to Sanity Home Page document.

## Brand

- [ ] Add ANRF logo when available.
- [ ] Add Naya Nari logo when available.
- [ ] Finalize color palette after logo review.
- [ ] Finalize typography after brand review.
- [ ] Replace temporary Unsplash imagery with ANRF field photos/video stills.

## Content

- [ ] Rewrite homepage copy.
- [ ] Rewrite About page copy.
- [ ] Rewrite Rosewood Conservation and Social Support pages.
- [ ] Add real contact details.
- [ ] Add documentation/blog/video/photo story content model examples.
- [x] Add final YouTube and Instagram URLs.
- [ ] Decide between simple social links, latest-video embed, or curated CMS-managed social highlights.

## SEO

- [x] Add page-specific metadata titles and descriptions.
- [ ] Add Open Graph image after final hero/logo assets exist.
- [x] Add sitemap and robots verification.
- [ ] Add structured data for nonprofit/organization details.

## Analytics

- [x] Add Vercel Web Analytics package/hook.
- [ ] Enable Vercel Web Analytics in Vercel project settings.
- [ ] Decide whether Google Analytics is needed later.
- [ ] Add basic conversion events after CTAs/forms are final.

## Launch

- Test Vercel preview URLs.
- Test CMS editing workflow.
- Connect custom domain after content and CMS are stable.
